...
def zeros(tupl, dtype):
    return 0


@overload(zeros)
def zeros_overload(tupl, dtype):
    config = utils.ConfigStack().top_or_none()
    if config.auto_parallel.enabled:
        def impl(tupl, dtype):
            parfors.parfor.init_prange()
            a = np.empty(tupl, dtype)
            for i in pndindex(a.shape):
                a[i] = 0
            return a
    else:
        def impl(tupl, dtype):
            return np.zeros(tupl, dtype)
    return impl
...
def np_arr_sum_overload(self, axis=None, dtype=None):
    ...
    config = utils.ConfigStack().top_or_none()
    is_parallel = config.auto_parallel.enabled
    ...
    def sum_impl(self, axis=None, dtype=None):
        ...
        if flag == True:
            res = zeros((1,), my_dtype)
            if is_parallel:
                for i in pndindex(self.shape):
                    res[0] += my_dtype(self[i]) if not is_timedelta else self[i]
            else:
                for v in np.nditer(self):
                    res[0] += my_dtype(v.item()) if not is_timedelta else v.item()
            return res[0]
        self_shape = self.shape
        ashape = list(self_shape)
        ashape.pop(real_axis)
        ashape_without_axis = _create_tuple_result_shape(ashape, self_shape)
        res = zeros(ashape_without_axis, dtype = my_dtype)
        if is_literal_value:
            temp = self
        else:
            ...
            temp = transpose_array
        for j in range(temp.shape[0]):
            for i in pndindex(temp[j].shape):
                res[i] += my_dtype(temp[j][i]) if not is_timedelta else temp[j][i]
        return res
    return sum_impl   
